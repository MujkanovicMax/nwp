...
def mkfc(values,f,u,v,D,dx,dy,idnum):
    
    
    #a=u[:,30:61]
    #c=v[:,30:61]
    vo6hsave = np.zeros((values.shape[0],values.shape[1],28))
    umean = np.nanmean(u)
    ...
    for j,time in enumerate(np.arange(1*dt,t,dt)):
            ...
            if time%(3600*6) == 0:
                ...
                map3.drawmapboundary(color='k', linewidth=1.0, fill_color=None)
                map3.drawcoastlines(linewidth=1.0, linestyle='solid', color='k')
                map3.contourf(coord[0],coord[1],dvalues.transpose(),100)
        
                plt.tight_layout()
                    
                plt.savefig("./"+str(idnum)+"/vo_"+str(time/3600.)+"_h_"+str(idnum)+".png")
                
                vo6hsave[:,:,int(time)/3600/6-1] = dvalues 
                
    return dvalues,vo6hsave
...
vo1 = ddx(v,dx) - ddy(u,dy)
vo2 = ddx(v,dx) - ddy(u+u_error,dy) + vo_error
...
vo_fc1,vo6hsave1 = mkfc(vo1,f,u,v,D,dx,dy,1)
vo_fc2,vo6hsave2 = mkfc(vo2,f,u+u_error,v,D,dx,dy,2)

lats=lats[ind_x]
ind2 = np.where((lats>=30)&(lats<=60))

RMSE = np.sqrt(np.mean(np.mean((vo6hsave1[ind2[0],:,:]-vo6hsave2[ind2[0],:,:])**2,axis=1),axis=0))

RMSE[1:] = RMSE[0:-1]
RMSE[0] = np.sqrt(np.mean(np.mean((vo1[ind2[0],:]-vo2[ind2[0],:])**2,axis=1),axis=0))
RMSE = np.append(RMSE, np.sqrt(np.mean(np.mean((vo_fc1[ind2[0],:]-vo_fc2[ind2[0],:])**2,axis=1),axis=0)))

fig,ax=plt.subplots()

ax.semilogy(np.arange(0,168+6,6),RMSE)
ax.set_xlabel("Leadtime in h")
ax.set_ylabel("RMSE")
ax.grid()
plt.tight_layout()
...
vodiff = [0]*29
vodiff[0] = vo1-vo2
vodiff[1:-1] = vo6hsave1[:,:,0:-1] - vo6hsave2[:,:,0:-1]
vodiff[-1] = vo_fc1 - vo_fc2

fig,ax=plt.subplots()
for i,p in enumerate(vodiff):
    map3 = Basemap(projection='cyl',llcrnrlat=0,urcrnrlat=90,\
                llcrnrlon=-180,urcrnrlon=180,resolution='c')
    map3.drawmapboundary(color='k', linewidth=1.0, fill_color=None)
    map3.drawcoastlines(linewidth=1.0, linestyle='solid', color='k')
    map3.contourf(coord[0],coord[1],p.transpose(),100)

    plt.tight_layout()
        
    plt.savefig("./"+"diff"+"/vodiff_"+str(i*6)+"_h_"+".png")
